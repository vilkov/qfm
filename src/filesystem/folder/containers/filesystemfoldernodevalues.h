...
public:
	struct Value
	{
		Value() :
			...
			item(0)
		{}
		Value(FileSystemBaseItem *item) :
			node(0),
			item(item)
		{
			Q_ASSERT(item);
		}
		Value(FileSystemBaseItem *item, Node *node) :
			node(node),
			item(item)
		{
			Q_ASSERT(item);
	...
	typedef ::Tools::Containers::HashedList<QString, Value> ValueList;
...
public:
	...
	size_type indexOf(FileSystemBaseItem *item) const
	{
		for (ValueList::size_type i = 0, size = m_items.size(); i < size; ++i)
	...
	void add(FileSystemBaseItem *item) { m_items.add(item->fileName(), item); }
	void add(FileSystemBaseItem *item, Node *node) { m_items.add(item->fileName(), Value(item, node)); }
