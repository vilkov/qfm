...
PerformCopyEntryTask::PerformCopyEntryTask(Node *node, FolderNodeEntry *entry, INode *destination, bool move) :
	parent_class(new Params(node, entry, destination, move), node),
	m_tryAgain(false),
...
void PerformCopyEntryTask::copyFile(IFileInfo *destination, FolderNodeEntry *entry, const volatile bool &stopedFlag)
{
	bool ok;
	IFile *sourceFile;
...
void PerformCopyEntryTask::askForOverwrite(const QString &title, const QString &text, const volatile bool &stopedFlag)
{
...
void PerformCopyEntryTask::askForSkipIfNotCopy(const QString &title, const QString &text, const volatile bool &stopedFlag)
{
	QuestionAnswerEvent::Params::Result result;
	QScopedPointer<QuestionAnswerEvent> event(new QuestionAnswerEvent());
...
QString PerformCopyEntryTask::Messages::failedToCopy(IFileInfo *source, IFileInfo *destination, const QString &error)
{
...
QString PerformCopyEntryTask::Messages::alreadyExists(IFileInfo *source, IFileInfo *destination)
{
...
bool PerformCopyEntryTask::doCopyFile(IFileInfo *destination, FolderNodeEntry *entry, IFile *sourceFile, const volatile bool &stopedFlag)
{
	bool res = false;
