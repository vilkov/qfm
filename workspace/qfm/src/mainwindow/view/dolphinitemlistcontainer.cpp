    ...
    m_fileItemListView(0)
{
    ...
    m_fileItemListView->setWidgetCreator(new KItemListWidgetCreator<KFileItemListWidget>());
    m_fileItemListView->setEnabledSelectionToggles(true);
...
void DolphinItemListContainer::updateGridSize()
{
    ...
    const int iconSize = 16;//previewsShown() ? settings.previewSize() : settings.iconSize();
    m_zoomLevel = 0;//ZoomLevelInfo::zoomLevelForIconSize(QSize(iconSize, iconSize));
    ...
    case KFileItemListView::IconsLayout: {
        ...
        itemWidth = minItemWidth + /* IconsModeSettings::textWidthIndex() */ 1 * 64;
...
{
    const int delay = -1; //GeneralSettings::autoExpandFolders() ? 750 : -1;
